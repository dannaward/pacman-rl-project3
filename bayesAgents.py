...
def fillYCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    yFactor.setProbability({Y_POS_VAR: BOTH_TOP_VAL}, PROB_BOTH_TOP)  # top
    yFactor.setProbability({Y_POS_VAR: BOTH_BOTTOM_VAL}, PROB_BOTH_BOTTOM)  # bottom
    yFactor.setProbability({Y_POS_VAR: LEFT_TOP_VAL}, PROB_ONLY_LEFT_TOP)  # leftTop
    yFactor.setProbability({Y_POS_VAR: LEFT_BOTTOM_VAL}, PROB_ONLY_LEFT_BOTTOM)  # leftBottom
    "*** END YOUR CODE HERE ***"
...
class VPIAgent(BayesAgent):
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, evid in self.getExplorationProbsAndOutcomes(evidence):
            expectedValue += prob * max(self.computeEnterValues(evid, enterEliminationOrder))
        "*** END YOUR CODE HERE ***"
